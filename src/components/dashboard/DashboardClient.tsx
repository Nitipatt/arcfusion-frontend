...
export function DashboardClient() {
  ...
  const loadStories = async (searchQuery: string = "") => {
    setLoading(true);
    setError(null);
    try {
      const storiesData = await fetchStories(searchQuery);
      setData(storiesData);
    } catch (err) {
      console.error(err);
  ...
  const hasStories =
    data &&
    (data.latest_stories.length > 0 || data.older_stories.length > 0);
      ...
      <div className="mx-auto max-w-6xl px-8 py-8">
        ...
        {error && (
          <div className="mb-8 rounded-2xl bg-red-50 p-6 text-center ring-1 ring-red-100 animate-fade-in">
            ...
            <button
              onClick={() => loadStories(globalSearch)}
        ...
        {/* Latest Updates */}
        {(loading || (hasStories && data!.latest_stories.length > 0)) && (
                ...
                {data?.latest_stories
                  .map((story, i) => (
                    <StoryCard
                      ...
                      story={story}
                      index={i}
                      onDelete={() => loadStories(globalSearch)}
                    />
                  ))}
              </div>
            )}
        ...
        {/* Older Stories */}
        {(loading || (hasStories && data!.older_stories.length > 0)) && (
          <section className="mb-12">
            <h2 className="mb-5 text-lg font-semibold text-slate-700">
              Older Stories
            </h2>
            {loading ? (
              <div className="flex gap-4 overflow-x-auto pb-2">
                {[0, 1, 2, 3].map((i) => (
                  <div
                    key={i}
                    className="min-w-[260px] rounded-2xl bg-blue-50 p-5"
                  >
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-thin">
                {data?.older_stories
                  .map((story, i) => (
                    <OlderStoryCard key={story.id} story={story} index={i} />
                  ))}
              </div>
            )}
          </section>
